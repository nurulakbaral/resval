...
export function setBreakpoints(
  ...
  if (!isArrayOfCSSUnits(breakpointsValues)) {
    throw new TypeError(
      'When you initialize `createResponsiveValues`, `breakpoints` property must contain CSS Units such as `px`, `rem`, `em`, etc and do not mix up string values and number values. Docs: developer.mozilla.org/en-US/docs/Learn/CSS/Building_blocks/Values_and_units',
    )
  }
