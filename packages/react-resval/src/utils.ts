...
import type { TBaseObject, TCSSAbsoluteUnits, TCSSRelativeUnits } from './types'

export function isEmptyObject<TTypeObject extends TBaseObject>(obj: TTypeObject) {
  return Object.keys(obj).length === 0 && obj.constructor === Object
}

export function isArrayOfNumber(arr: number[]): boolean {
  return arr.every((value) => typeof Number(value) === 'number' && !Number.isNaN(Number(value)))
}

export function isArrayOfCSSUnits(arr: string[]): boolean {
